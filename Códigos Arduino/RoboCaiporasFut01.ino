...
WiFiClient espClient2; PubSubClient client(espClient2);
...
void callback(String topic, byte* message, unsigned int length) {  Serial.print("Message arrived on topic: ");  
  ...
  if(topic=="Mover01"){
    ...
    else if( (messageAtual == "gira_horario") ){ Carrinho.dir();delay(50); Carrinho.stopGERAL();}

    else if( (messageAtual == "gira_antihorario") ){ Carrinho.esq();delay(50); Carrinho.stopGERAL();}
  }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {  Serial.println("connected");
      ...
      client.subscribe("Mover01"); // topico em que o dispositivo esta inscrito
