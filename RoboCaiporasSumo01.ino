...
void callback(String topic, byte* message, unsigned int length) {  Serial.print("Message arrived on topic: ");  
  ...
  if(topic=="Mover03"){
    if( (messageAtual == "frente") ){ Carrinho.frente(); delay(200); Carrinho.stopGERAL(); }
    
    else if( (messageAtual == "reverso") ){ Carrinho.retorno();delay(200); Carrinho.stopGERAL();}
    
    else if( (messageAtual == "gira_horario") ){ Carrinho.dir();delay(200); Carrinho.stopGERAL();}

    else if( (messageAtual == "gira_antihorario") ){ Carrinho.esq();delay(200); Carrinho.stopGERAL();}
  }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {  Serial.println("connected");
      ...
      client.subscribe("Mover03"); // topico em que o dispositivo esta inscrito
